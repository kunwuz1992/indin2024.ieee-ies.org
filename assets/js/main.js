(function() {
	...
	ScheduleTemplate.prototype.initEvents = function() {
		...
		for(var i = 0; i < this.singleEvents.length; i++) {
            ...
            // aluque 2019-04-26. Hide content in the main schedule
            this.singleEvents[i].getElementsByTagName('a')[0].getElementsByClassName("cd-schedule__content")[0].setAttribute("style","display: none");
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		this.modal.setAttribute('data-event', target.getAttribute('data-event'));
                ...
                // aluque 2019-04-26. Use the element cd-schedule__content as content instead of loading from external HTML
                this.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = target.getElementsByClassName("cd-schedule__content")[0].innerHTML;
                Util.addClass(this.modal, 'cd-schedule-modal--content-loaded');

		//update event content. Commented out by aluque 2019-04-26
		...
		if( mq == 'mobile' ) {
			...
			var eventPosition = target.getBoundingClientRect(),
				...
				eventWidth = Math.min(target.offsetWidth,200);
	...
	if( scheduleTemplate.length > 0 ) { // init ScheduleTemplate objects
		...
		var origScheduleRowsHeight,origTextXL,origTextSM;
		window.addEventListener("beforeprint", function(event) {
                        origScheduleRowsHeight=document.documentElement.style.getPropertyValue('--schedule-rows-height');
                        origTextXL=document.documentElement.style.getPropertyValue('--text-xl');
                        origTextSM=document.documentElement.style.getPropertyValue('--text-sm');
                        // approx size for schedule in an A4 is 480px 
                        // (height in landscape is 595px at 72dpi, less margins, date text, etc.)
                        var newheight=Math.floor(480/document.documentElement.style.getPropertyValue('--schedule-rows-number'));
			document.documentElement.style.setProperty('--schedule-rows-height', newheight+'px');
                        document.documentElement.style.setProperty('--text-xl', '25px');
		...
		window.addEventListener("afterprint", function(event) {
                    console.log(origScheduleRowsHeight);
			document.documentElement.style.setProperty('--schedule-rows-height',origScheduleRowsHeight);
                        document.documentElement.style.setProperty('--text-xl',origTextXL);
                        document.documentElement.style.setProperty('--text-sm',origTextSM);                        
			for(var i = 0; i < scheduleTemplateArray.length; i++) {
				scheduleTemplateArray[i].placeEvents();
			}
		});
